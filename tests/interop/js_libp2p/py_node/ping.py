    ...
    try:
        while True:
            try:
                ...
                if not data:
                    print(f"[INFO] No data received, connection likely closed by {peer_id}")
                ...
                ping_count += 1
                print(f"[PING {ping_count}] Received ping from {peer_id}: {len(data)} bytes")
...
Examples:
  ...
  python ping.py client {example_maddr} --count 10
    ...
    client_parser.add_argument('--count', '-c', type=int, default=5,
